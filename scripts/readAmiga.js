...
WebTracker.readAmigaSample = function (buffer, ptrs) {
	...
	var dataView = (buffer instanceof ArrayBuffer) ? new DataView(buffer) : buffer,
		...
		readHeader = function () {
			ins.title = WebTracker.stringReader(dataView)(hoff, 22);
			hoff += 22;
		...
		readData = function () {
			...
			smp.sampleRate = 8287.2; //standard amiga sample sampleRate.
		}; //readData

	readHeader();
	readData();
	smp.loopStart = lstart;
	smp.loopEnd = lend;
ins.addSample(smp);
ins.mapSample(); //automatically maps the first sample to all notes.
	ptrs.headerOffset = hoff;
	ptrs.dataOffset = doff;
	return ins; //return the newly loaded sample
}; //readAmigaSample
